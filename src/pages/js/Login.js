...
export default function Login() {
  ...
  function userModal() {
    ...
    axios.post(`${url}/auth/register/`, formdata).then(res => {
      ...
      console.log(err);
      { state1 === "eng" ? (alert("Enter the full information")) : (alert("Введите полную информацию")) }
    })
  ...
  function userVeri() {
    ...
    axios.post(`${url}/auth/register/?verify_code=${name.target.value}&email=${email.target.value}`).then(res => {
      { state1 === "eng" ? (alert("You have registered")) : (alert("Вы зарегистрировались")) }
      setPage(1)
    }).catch(err => {
      console.log(err);
      { state1 === "eng" ? (alert("Verification error ")) : (alert("Ошибка верификации ")) }
    })
  }
  ...
  function userAvto() {
    ...
    axios.post(`${url}/auth/login/`, formdata).then(res => {
      ...
      console.log(err);
      { state1 === "eng" ? (alert("There is no such login or password in the database")) : (alert("В базе нет такого логина или пароля")) }
    })
  }
